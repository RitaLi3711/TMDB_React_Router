    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
      {credits.map((item: any) => (
        ...
        >
          ...
          />
          <div className="p-2">
            <p className="text-white text-sm font-medium line-clamp-1">{item.title || item.name}</p>
            <p className="text-gray-400 text-xs line-clamp-1">{item.character || item.job || ''}</p>
          </div>
        </button>
      ))}
    </div>
